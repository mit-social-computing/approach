<div class="border">
	<div class="border">
		...
		<?php else : ?>
			<p class="last-check">Last Add-on Check: <?php echo date('l, M. j, Y @ g:ia.', $last_check) ?>&nbsp;&nbsp;<a href="<?php echo BASE . AMP . 'C=addons_accessories' . AMP . 'M=process_request' . AMP . 'accessory=lamplighter' . AMP . 'method=process_refresh' ?>" class="available refresh">Check Now</a>&nbsp;<img class="monitor-loading" src="<?php echo defined("URL_THIRD_THEMES") ? URL_THIRD_THEMES : '/themes'; ?>/lamplighter/images/loader.gif" alt="Loading..." /></p>
			<table>
				<thead>
					<tr class="first">
						...
						<?php endif ?>
							...
							<td class="addon-name">
								<?php if (! in_array($package, $hidden_addons) ): ?>
									<?php echo $addon->name ?><span><?php echo $commercial_designation ?>&nbsp;<a class="hide_addon" href="<?php echo BASE . AMP . 'C=addons_accessories' . AMP . 'M=process_request' . AMP . 'accessory=lamplighter' . AMP . 'method=process_hide_addon' . AMP . 'package=' . $package ?>">Hide this</a></span>
								<?php else: ?>
									<?php echo $addon->name ?><span><?php echo $commercial_designation ?>&nbsp;<a class="unhide_addon" href="<?php echo BASE . AMP . 'C=addons_accessories' . AMP . 'M=process_request' . AMP . 'accessory=lamplighter' . AMP . 'method=process_unhide_addon' . AMP . 'package=' . $package ?>">Unhide this</a></span>
								<?php endif; ?>
							</td>
		...
		<div id="lamplighter-footer">
			<?php if( ! empty($hidden_addons) && $show_hidden == FALSE) : ?>
				<p class="hidden"><a href="<?php echo BASE . AMP . 'C=addons_accessories' . AMP . 'M=process_request' . AMP . 'accessory=lamplighter' . AMP . 'method=process_display_hidden_addons' ?>" class="show-hidden-addons">Show Hidden Add-ons (<?php echo count($hidden_addons) ?>)</a></p>
			<?php elseif ( ! empty($hidden_addons) && $show_hidden == TRUE) : ?>
				<p class="hidden"><a href="<?php echo BASE . AMP . 'C=addons_accessories' . AMP . 'M=process_request' . AMP . 'accessory=lamplighter' . AMP . 'method=process_refresh' ?>" class="show-hidden-addons">Hide Previously Hidden Add-ons (<?php echo count($hidden_addons) ?>)</a></p>
			<?php endif ?>
			<p class="Check">Last Add-on Check: <?php echo date('l, M. j, Y @ g:ia.', $last_check) ?>&nbsp;&nbsp;<a href="<?php echo BASE . AMP . 'C=addons_accessories' . AMP . 'M=process_request' . AMP . 'accessory=lamplighter' . AMP . 'method=process_refresh' ?>" class="available refresh">Check Now</a>&nbsp;<img class="monitor-loading" src="<?php echo defined("URL_THIRD_THEMES") ? URL_THIRD_THEMES : '/themes'; ?>/lamplighter/images/loader.gif" alt="Loading..." /></p>
			<p class="logos">
				<a href="<?php echo $cp->masked_url('https://lamplighter.io/?utm_source=devotee_monitor&utm_medium=text_link&utm_content=add-on&utm_campaign=expressionengine'); ?>" target="_blank" class="first">Lamplighter.io</a>
			</p>
			<p>
				<small><a href="<?php echo BASE . AMP . 'C=addons_accessories' . AMP . 'M=process_request' . AMP . 'accessory=lamplighter' . AMP . 'method=process_debug_info' ?>" target="_blank" >Site debug info &rarr;</a></small>
			</p>
		</div>
	</div><!-- /.border -->
</div><!-- /.border -->