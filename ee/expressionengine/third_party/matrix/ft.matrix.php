...
class Matrix_ft extends EE_Fieldtype {
	...
	function update($from)
	{
		...
		if (version_compare($from, '2.5', '<'))
		{
			...
			if (!$this->EE->db->field_exists('is_draft', 'exp_matrix_data'))
			{
				$new_field = array(
					'is_draft' => array('type' => 'TINYINT', 'constraint' => '1', 'unsigned' => TRUE, 'default' => 0),
				);
		...
		if (! isset($this->cache['field_cols'][$cache_key]))
		{
			...
			else
			{
				...
				foreach ($cols as &$col)
				{
					...
					$col['celltype_settings'] = array_merge($this->settings, $celltype->settings, $col['col_settings'], array(
						'col_id'       => $col['col_id'],
	...
	function display_field($data)
	{
		...
		if (!is_array($data) && base64_decode($data))
		{
			$unseralized = @unserialize(base64_decode($data));
			if (is_array($unseralized))
			{
				$data = $unseralized;

				$current_data = $this->EE->db->select('*')->where('entry_id', $this->entry_id)->where('field_id', $this->field_id)->get('matrix_data')->result();
		...
		else
		{
			...
			else
			{
				...
				{
					if ($this->entry_id && !empty($this->EE->session->cache['ep_better_workflow']['is_draft']))
					{
						$this->is_draft = 1;
						if (!isset($this->EE->session->cache['ep_better_workflow']['preview_entry_data']))
						{
							$this->EE->session->cache['ep_better_workflow']['preview_entry_data'] = new StdClass;
						}
						$this->EE->session->cache['ep_better_workflow']['preview_entry_data']->entry_id = $this->entry_id;
					}
	...
	private function _save($data)
	{
		...
		//
		if ($this->EE->extensions->active_hook('matrix_save_field'))
		{
			$data = $this->EE->extensions->call('matrix_save_field', $this, $data);
		}
	...
	private function _data_query($params = array(), $select_mode = 'data', $select_aggregate = '')
	{
		...
		if (!empty($this->EE->session->cache['ep_better_workflow']['is_draft'])
			&& isset($this->EE->session->cache['ep_better_workflow']['preview_entry_data']->entry_id)
			&& $this->EE->session->cache['ep_better_workflow']['preview_entry_data']->entry_id == $this->entry_id)
		{
			$this->is_draft = 1;
	 	}
